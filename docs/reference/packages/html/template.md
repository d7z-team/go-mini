...
Package template (html/template) implements data-driven templates for generating HTML output safe against code injection. It provides the same interface as [text/template](../text/template.md) and should be used instead of [text/template](../text/template.md) whenever the output is HTML.

The documentation here focuses on the security features of the package. For information about how to program the templates themselves, see the documentation for [text/template](../text/template.md).

#### Introduction

This package wraps [text/template](../text/template.md) so you can share its template API to parse and execute HTML templates safely.
...
Comments are stripped from output, except for those passed in via the [HTML](template.md#type-html), [CSS](template.md#type-css), and [JS](template.md#type-js) types for their respective contexts.
...
- [type CSS](#type-css)
- [const ErrAmbigContext](#const-errambigcontext)
- [const ErrBadHTML](#const-errbadhtml)
- [const ErrBranchEnd](#const-errbranchend)
- [const ErrEndContext](#const-errendcontext)
- [const ErrJSTemplate](#const-errjstemplate)
- [const ErrNoSuchTemplate](#const-errnosuchtemplate)
- [const ErrOutputContext](#const-erroutputcontext)
- [const ErrPartialCharset](#const-errpartialcharset)
- [const ErrPartialEscape](#const-errpartialescape)
- [const ErrPredefinedEscaper](#const-errpredefinedescaper)
- [const ErrRangeLoopReentry](#const-errrangeloopreentry)
- [const ErrSlashAmbig](#const-errslashambig)
- [type Error](#type-error)
  - [Error.Error](#func-errorerror)
- [type ErrorCode](#type-errorcode)
- [type FuncMap](#type-funcmap)
- [type HTML](#type-html)
- [type HTMLAttr](#type-htmlattr)
- [func HTMLEscape](#func-htmlescape)
- [func HTMLEscapeString](#func-htmlescapestring)
- [func HTMLEscaper](#func-htmlescaper)
- [func IsTrue](#func-istrue)
- [type JS](#type-js)
- [func JSEscape](#func-jsescape)
- [func JSEscapeString](#func-jsescapestring)
- [func JSEscaper](#func-jsescaper)
- [type JSStr](#type-jsstr)
- [func Must](#func-must)
- [func New](#func-new)
- [const OK](#const-ok)
- [func ParseFS](#func-parsefs)
- [func ParseFiles](#func-parsefiles)
- [func ParseGlob](#func-parseglob)
- [type Srcset](#type-srcset)
- [type Template](#type-template)
  - [Template.AddParseTree](#func-templateaddparsetree)
  - [Template.Clone](#func-templateclone)
  - [Template.DefinedTemplates](#func-templatedefinedtemplates)
  - [Template.Delims](#func-templatedelims)
  - [Template.Execute](#func-templateexecute)
  - [Template.ExecuteTemplate](#func-templateexecutetemplate)
  - [Template.Funcs](#func-templatefuncs)
  - [Template.Lookup](#func-templatelookup)
  - [Template.Name](#func-templatename)
  - [Template.New](#func-templatenew)
  - [Template.Option](#func-templateoption)
  - [Template.Parse](#func-templateparse)
  - [Template.ParseFS](#func-templateparsefs)
  - [Template.ParseFiles](#func-templateparsefiles)
  - [Template.ParseGlob](#func-templateparseglob)
  - [Template.Templates](#func-templatetemplates)
- [type URL](#type-url)
- [func URLQueryEscaper](#func-urlqueryescaper)
...
ErrBranchEnd: "{{if}} branches end in different contexts" Examples:
...
ErrJSTemplate: "... appears in a JS template literal" Example:
...
Deprecated: ErrJSTemplate is no longer returned when an action is present in a JS template literal. Actions inside of JS template literals are now escaped as expected.
...
Must is a helper that wraps a call to a function returning ([\*Template](template.md#type-template), error) and panics if the error is non-nil. It is intended for use in variable initializations such as
...
ParseFS is like [ParseFiles](template.md#func-parsefiles) or [ParseGlob](template.md#func-parseglob) but reads from the file system fs instead of the host operating system's file system. It accepts a list of glob patterns. (Note that most file names serve as glob patterns matching only themselves.)
...
ParseFiles creates a new [Template](template.md#type-template) and parses the template definitions from the named files. The returned template's name will have the (base) name and (parsed) contents of the first file. There must be at least one file. If an error occurs, parsing stops and the returned [\*Template](template.md#type-template) is nil.
...
ParseGlob creates a new [Template](template.md#type-template) and parses the template definitions from the files identified by the pattern. The files are matched according to the semantics of filepath.Match, and the pattern must match at least one file. The returned template will have the (base) name and (parsed) contents of the first file matched by the pattern. ParseGlob is equivalent to calling [ParseFiles](template.md#func-parsefiles) with the list of files matched by the pattern.
...
Node is the node that caused the problem, if known. If not nil, it overrides Name and Line.
...
Error returns the template name, source position, and error description.
...
FuncMap maps template function names to implementations.
...
Clone returns a duplicate of the template, including all associated templates. The actual representation is not copied, but the name space of associated templates is, so further calls to [Template.Parse](template.md#func-templateparse) in the copy will add templates to the copy but not to the original. [Template.Clone](template.md#func-templateclone) can be used to prepare common templates and use them with variant definitions for other templates by adding the variants after the clone is made.
...
Delims sets the action delimiters to the specified strings, to be used in subsequent calls to [Template.Parse](template.md#func-templateparse), [ParseFiles](template.md#func-parsefiles), or [ParseGlob](template.md#func-parseglob). Nested template definitions will inherit the settings. An empty delimiter stands for the corresponding default: {{ or }}. The return value is the template, so calls can be chained.
...
Templates can be redefined in successive calls to Parse, before the first use of [Template.Execute](template.md#func-templateexecute) on t or any associated template. A template definition with a body containing only white space and comments is considered empty and will not replace an existing template's body. This allows using Parse to add new named template definitions without overwriting the main template body.
...
ParseFS is like [Template.ParseFiles](template.md#func-templateparsefiles) or [Template.ParseGlob](template.md#func-templateparseglob) but reads from the file system fs instead of the host operating system's file system. It accepts a list of glob patterns. (Note that most file names serve as glob patterns matching only themselves.)
