...
The [New](errors.md#func-new) function creates errors whose only content is a text message.
...
An easy way to create wrapped errors is to call [fmt.Errorf](fmt.md#func-errorf) and apply the %w verb to the error argument:

	wrapsErr := fmt.Errorf("... %w ...", ..., err, ...)

Successive unwrapping of an error creates a tree. The [Is](errors.md#func-is) and [As](errors.md#func-as) functions inspect an error's tree by examining first the error itself followed by the tree of each of its children in turn (pre-order, depth-first traversal).
...
[Is](errors.md#func-is) examines the tree of its first argument looking for an error that matches the second. It reports whether it finds a match. It should be used in preference to simple equality checks:
...
because the former will succeed if err wraps [io/fs.ErrExist](io/fs.md#var-errexist).

[AsType](errors.md#func-astype) examines the tree of its argument looking for an error whose type matches its type argument. If it succeeds, it returns the corresponding value of that type and true. Otherwise, it returns the zero value of that type and false. The form
...
because the former will succeed if err wraps an [\*io/fs.PathError](io/fs.md#type-patherror).

## Index

- [func As](#func-as)
- [func AsType](#func-astype)
- [var ErrUnsupported](#var-errunsupported)
- [func Is](#func-is)
- [func Join](#func-join)
- [func New](#func-new)
...
ErrUnsupported indicates that a requested operation cannot be performed, because it is unsupported. For example, a call to [os.Link](os.md#link) when using a file system that does not support hard links.
...
either by directly wrapping ErrUnsupported or by implementing an [Is](errors.md#func-is) method.
...
For most uses, prefer [AsType](errors.md#func-astype). As is equivalent to [AsType](errors.md#func-astype) but sets its target argument rather than returning the matching error and doesn't require its target argument to implement error.
...
then Is(MyError{}, fs.ErrExist) returns true. See syscall.Errno.Is for an example in the standard library. An Is method should only shallowly compare err and the target and not call [Unwrap](errors.md#func-unwrap) on either.
...
A non-nil error returned by Join implements the Unwrap() \[]error method. The errors may be inspected with [Is](errors.md#func-is) and [As](errors.md#func-as).
...
Unwrap only calls a method of the form "Unwrap() error". In particular Unwrap does not unwrap errors returned by [Join](errors.md#func-join).
