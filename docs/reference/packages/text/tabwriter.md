...
- [const AlignRight](#const-alignright)
- [const Debug](#const-debug)
- [const DiscardEmptyColumns](#const-discardemptycolumns)
- [const Escape](#const-escape)
- [const FilterHTML](#const-filterhtml)
- [func NewWriter](#func-newwriter)
- [const StripEscape](#const-stripescape)
- [const TabIndent](#const-tabindent)
- [type Writer](#type-writer)
  - [Writer.Flush](#func-writerflush)
  - [Writer.Init](#func-writerinit)
...
NewWriter allocates and initializes a new [Writer](tabwriter.md#type-writer). The parameters are the same as for the Init function.
...
If [DiscardEmptyColumns](tabwriter.md#const-discardemptycolumns) is set, empty columns that are terminated entirely by vertical (or "soft") tabs are discarded. Columns terminated by horizontal (or "hard") tabs are not affected by this flag.
...
A segment of text may be escaped by bracketing it with [Escape](tabwriter.md#const-escape) characters. The tabwriter passes escaped text segments through unchanged. In particular, it does not interpret any tabs or line breaks within the segment. If the [StripEscape](tabwriter.md#const-stripescape) flag is set, the Escape characters are stripped from the output; otherwise they are passed through as well. For the purpose of formatting, the width of the escaped text is always computed excluding the Escape characters.

The formfeed character acts like a newline but it also terminates all columns in the current line (effectively calling [Writer.Flush](tabwriter.md#func-writerflush)). Tab- terminated cells in the next line start new columns. Unless found inside an HTML tag or inside an escaped text segment, formfeed characters appear as newlines in the output.

The Writer must buffer input internally, because proper spacing of one line may depend on the cells in future lines. Clients must call Flush when done calling [Writer.Write](tabwriter.md#func-writerwrite).
...
Flush should be called after the last call to [Writer.Write](tabwriter.md#func-writerwrite) to ensure that any data buffered in the [Writer](tabwriter.md#type-writer) is written to output. Any incomplete escape sequence at the end is considered complete for formatting purposes.
...
A [Writer](tabwriter.md#type-writer) must be initialized with a call to Init. The first parameter (output) specifies the filter output. The remaining parameters control the formatting:
