...
To generate HTML output, see [html/template](../html/template.md), which has the same interface as this package but automatically secures HTML output against certain attacks.
...
By construction, a template may reside in only one association. If it's necessary to have a template addressable from multiple associations, the template definition must be parsed multiple times to create distinct \*Template values, or must be copied with [Template.Clone](template.md#func-templateclone) or [Template.AddParseTree](template.md#func-templateaddparsetree).

Parse may be called multiple times to assemble the various associated templates; see [ParseFiles](template.md#func-parsefiles), [ParseGlob](template.md#func-parseglob), [Template.ParseFiles](template.md#func-templateparsefiles) and [Template.ParseGlob](template.md#func-templateparseglob) for simple ways to parse related templates stored in files.

A template may be executed directly or through [Template.ExecuteTemplate](template.md#func-templateexecutetemplate), which executes an associated template identified by name. To invoke our example above, we might write,
...
- [type ExecError](#type-execerror)
  - [ExecError.Error](#func-execerrorerror)
  - [ExecError.Unwrap](#func-execerrorunwrap)
...
Error returns the formatted template execution error.
...
Errors returned by Execute wrap the underlying error; call [errors.AsType](../errors.md#func-astype) to unwrap them.

When template execution invokes a function with an argument list, that list must be assignable to the function's parameter types. Functions meant to apply to arguments of arbitrary type can use parameters of type interface{} or of type [reflect.Value](../reflect.md#type-value). Similarly, functions meant to return a result of arbitrary type can return interface{} or [reflect.Value](../reflect.md#type-value).
...
Template is the representation of a parsed template. The \*parse.Tree field is exported only for use by [html/template](../html/template.md) and should be treated as unexported by all other clients.
...
DefinedTemplates returns a string listing the defined templates, prefixed by the string "; defined templates are: ". If there are none, it returns the empty string. For generating an error message here and in [html/template](../html/template.md).
...
If data is a [reflect.Value](../reflect.md#type-value), the template applies to the concrete value that the reflect.Value holds, as in [fmt.Print](../fmt.md#func-print).
