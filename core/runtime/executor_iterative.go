...
func NewIterativeExecutor(program *ast.ProgramStmt) (*IterativeExecutor, error) {
	if program == nil {
		return nil, errors.New("invalid program")
	}
	result := &IterativeExecutor{
		program: program,
...
func (e *IterativeExecutor) RegisterRoute(name string, route FFIRoute) {
	e.routes[name] = route
}

// 模拟原 Executor.ExecExpr 用于初始化全局变量 (临时回退机制，直至完全重构)
func (e *IterativeExecutor) ExecExpr(ctx *StackContext, s ast.Expr) (*Var, error) {
...
func (e *IterativeExecutor) Execute(ctx context.Context) (err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			slog.Error("IterativeExecutor panic", "error", r, "stack", string(debug.Stack()))
			if err == nil {
...
func (e *IterativeExecutor) handleUnwind(session *StackContext, task *Task) bool {
...
// 供解卷状态恢复使用
func (e *IterativeExecutor) dispatch(session *StackContext, task Task) error {
	switch task.Op {
	case OpExec:
	...
	case OpLoadVar:
		...
		if err != nil {
			exec := session.Executor.(*IterativeExecutor)
			if exec.program != nil {
	...
	case OpImportInit:
		...
		if e.Loader != nil {
			...
			if err == nil {
				...
				modExecutor, err := NewIterativeExecutor(prog)
				if err != nil {
	...
	case OpImportDone:
		...
		prog := modSession.Executor.(*IterativeExecutor).program
...
func (e *IterativeExecutor) handleExec(session *StackContext, stmt ast.Stmt) error {
	switch n := stmt.(type) {
	case *ast.BadStmt:
...
func (e *IterativeExecutor) evalLHS(session *StackContext, lhsExpr ast.Expr) error {
...
func (e *IterativeExecutor) scheduleForBody(session *StackContext, n *ast.ForStmt) {
	session.TaskStack = append(session.TaskStack, Task{Op: OpLoopBoundary, Node: n})
	if n.Update != nil {
...
func (e *IterativeExecutor) incDecLHSDesc(session *StackContext, lhsDesc interface{}, op string) error {
...
func (e *IterativeExecutor) assignToLHSDesc(session *StackContext, lhsDesc interface{}, val *Var) error {
	switch desc := lhsDesc.(type) {
	case *LHS_Env:
...
func (e *IterativeExecutor) handleEval(session *StackContext, expr ast.Expr) error {
	switch n := expr.(type) {
	case *ast.BadExpr:
...
func (e *IterativeExecutor) Run(session *StackContext) error {
...
func (e *IterativeExecutor) GetProgram() *ast.ProgramStmt {
	return e.program
}

func (e *IterativeExecutor) ExecuteStmts(session *StackContext, stmts []ast.Stmt) error {
...
func (e *IterativeExecutor) ImportModule(ctx *StackContext, n *ast.ImportExpr) (*Var, error) {
    oldTasks := ctx.TaskStack
    oldValues := ctx.ValueStack
