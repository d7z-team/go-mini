...
func buildMegaPipelineSpec(phases int) megaPipelineSpec {
	...
	writeLine("\t}")
	writeLine("\tt := spawn(func(base int) int {")
	writeLine("\t\treturn helper.Next() + base")
	writeLine("\t}, 5)")
	writeLine("\tchildSum = await(t)")
	writeLine("\tg := task.NewTaskGroup()")
	writeLine("\tt2 := spawn(func() int { return 2 })")
	writeLine("\ttask.AddTask(g, t2)")
	writeLine("\ttask.WaitTasks(g)")
	writeLine("\tif task.GroupErr(g) != nil {")
	writeLine(`		panic("unexpected task group error")`)
	writeLine("\t}")
	writeLine("\tif await(t2) != 2 {")
	writeLine(`		panic("unexpected task result")`)
	writeLine("\t}")
