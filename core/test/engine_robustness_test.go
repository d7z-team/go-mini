...
func TestTypeSystemEdgeCases(t *testing.T) {
	t.Run("DeeplyNestedArray", func(t *testing.T) {
		// Array<Array<Array<...>>>
		typeStr := "Array<Array<Array<Array<Number>>>>"
		...
		assert.True(t, ok)
		assert.Equal(t, ast.OPSType("Array<Array<Array<Number>>>"), elem)

		elem2, _ := elem.ReadArrayItemType()
		elem3, _ := elem2.ReadArrayItemType()
		elem4, _ := elem3.ReadArrayItemType()
		assert.Equal(t, ast.OPSType("Number"), elem4)
	})

	t.Run("ComplexMap", func(t *testing.T) {
		typeStr := "Map<String, Array<Ptr<Number>>>"
		miniType := ast.OPSType(typeStr)
		...
		assert.Equal(t, ast.OPSType("Array<Ptr<Number>>"), v)
	})
}
