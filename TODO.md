...
- [x] **收内部 runtime 类型桥接 API**: `Var.RawType()` 已改为返回 `TypeSpec`，`SetRawType(...)` 统一为 `string` 入口；`resolveNamedType*`、`resolveInterfaceSpec`、`resolveStructSchema`、`parseRuntimeStructType`、`parseRuntimeInterfaceType` 等内部 helper 已切到 `TypeSpec/string`，runtime 内部不再以 `ast.GoMiniType` 作为通用桥接类型。
...
- [x] **补充 FFI `bytes` 调用后 copy-back 语义**: 已引入显式 `inout bytes` 契约，Go 宿主侧通过 `*ffigo.BytesRef` 声明可回写参数；runtime/ffigen/host router 已支持单次 FFI 调用结束后的整体 copy-back，包含长度变化，并覆盖变量、成员、索引、解引用左值。默认 `TypeBytes` / `[]byte` 仍保持值传递，不自动升级为回写语义。
- [x] **补充通用 `array` copy-back 语义**: 已新增显式 `inout array` 契约，Go 宿主侧通过 `*ffigo.ArrayRef[T]` 声明容器级回写；runtime/ffigen/host router 已支持完整 `Array<T>` 左值的整体回写与长度变化。FFI 边界当前显式不支持 `buf[lo:hi]` / `arr[lo:hi]` 这类 slice/window 左值，避免引入 alias、窗口替换与回写顺序歧义。
- [x] **补齐 LSP 链式调用推导剩余缺口**: 已收口 `inferLSPTypeRecursive` / member completion 的递归 resolve 路径，覆盖 imported module 的 tuple-return 首元素启发式、命名 alias、命名接口方法链继续推导，并补齐专项 completion 回归。
- [x] **统一 AST 诊断结构化转发与 range 去重**: 已为 `Program/Block/Function/FuncLit/If/For/Switch/Return` 及 `binary/unary/inc-dec/index/slice/call/assignment` 等主路径统一接入 `ForwardStructuredError` / `AddErrorAt`，移除 `IfStmt Cond.Check error: ...`、`函数缺少返回语句` 这类包装型兜底消息；同消息诊断现在会优先保留更小、更精确的源码 range，并补齐 `core/ast/tests` 结构化转发、窄 range 优先与 fallback 落点回归。
- [x] **收敛复合字面量启发式推导精度**: `CompositeExpr` 启发式推导现已区分“真实动态 Any”与“被坏子表达式污染”的场景；污染子节点不再把整个容器无差别降成 `Map<..., Any>` / `Array<Any>`，并能保留可靠公共类型与精确错误来源。
- [x] **清理剩余 `Any` 宽容分支**: 已收紧 `StarExpr` / `IndexExpr` / `SliceExpr` / `MemberExpr` / `CompositeExpr` 的污染型 `Any` 放行，移除 concrete array/string/bytes/map key 与裸 `TypeHandle` 的静态兼容；仅保留真正动态 `Any` / `map[string] any` 容器访问语义，并补齐前端精确诊断与 runtime/e2e 回归。
...
- [x] **收口模块导入与全局初始化对 program shell / blueprint 的执行依赖**: `PreparedProgram` 现在是执行必需 metadata 的完整装载工件，包含 package、import alias、constants、named types、struct/interface schema、globals/functions/main task plan 与 global init order。`NewExecutorFromPrepared(...)` 已删除 AST 参数，只消费 prepared executable；`runtime.Executor.ModulePlanLoader` / `executeImportedProgram(...)` 也已收敛为 prepared-only。`bytecode.Program.Executable` 不再要求 `Blueprint` 才能通过校验，`ArtifactFromBytecode(...)` 仅在存在 blueprint 时重建 AST shell，纯 executable bytecode 可以直接恢复运行时。engine 侧 `RegisterModule(...)` / `modulePlanLoader(...)` 运行路径缓存 prepared executable，AST module loader 仅作为前端边界输入并在交给 runtime 前即时 `PrepareProgram`。
