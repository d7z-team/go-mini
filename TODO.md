...
- 模板函数支持全局保留名、运行时包成员和编译期虚拟包成员三类入口；真实包/member 与真实符号冲突由 compiler 模板计划统一校验，编译期虚拟包不需要 dummy module，fixed-point 展开后必须从 AST import、artifact imports 与 bytecode 中完全移除。
