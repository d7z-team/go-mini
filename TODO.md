...
- [ ] **补充通用 `array` / `slice` copy-back 语义**: 当前 copy-back 仅覆盖显式 `BytesRef` 对应的 `TypeBytes`。后续若要支持 `Array<T>` 或 `buf[lo:hi]` 这类窗口回写，需要单独定义容器级 `inout` schema、host helper、wire 编码和 VM 左值回写语义，特别是长度变化、窗口替换和重叠区间冲突规则。
