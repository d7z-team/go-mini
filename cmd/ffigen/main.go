...
func (r *schemaRegistry) Ensure(displayName, specLiteral string) string {
	...
	if idx, ok := r.byVar[varName]; ok {
		if len(specLiteral) > len(r.ordered[idx].specLiteral) {
			r.ordered[idx].specLiteral = specLiteral
			r.ordered[idx].displayName = displayName
		}
		return varName
	}
...
func main() {
	...
	for _, target := range targets {
		interfaces = append(interfaces, generateCode(*pkgName, target.spec, structs, target.meta, globalConsts, schemas))
	}
	...
	fullInterfaces := schemaDefs.String() + strings.Join(interfaces, "\n")
...
func generateCode(pkg string, spec *ast.TypeSpec, structs map[string]*ast.StructType, meta targetMeta, constants map[string]string, schemas *schemaRegistry) string {
	name := spec.Name.Name
	...
	for _, structName := range referencedStructs {
		referencedSchemaVars[structName] = schemas.Ensure(
	...
	if isStruct && methodsPrefix != "" {
		structSchemaVar = schemas.Ensure(
			displayTypeName(name),
			buildStructSchemaLiteral(name, true, true),
		)
	} else if methodsPrefix != "" {
		includeFields := structs[methodsPrefix] != nil
		structSchemaVar = schemas.Ensure(
			displayTypeName(methodsPrefix),
			buildStructSchemaLiteral(methodsPrefix, includeFields, true),
		)
	}
	...
	if isStruct && methodsPrefix != "" {
		...
		fmt.Fprintf(&sb, "\t\tif checker, ok := executor.(interface{ HasStructSchema(string) bool }); ok && checker.HasStructSchema(name) {\n")
		fmt.Fprintf(&sb, "\t\t\treturn\n")
		fmt.Fprintf(&sb, "\t\t}\n")
		...
		for _, structName := range referencedStructs {
			...
			fmt.Fprintf(&sb, "\tregisterStructSchema(\"%s\", %s)\n", displayTypeName(structName), referencedSchemaVars[structName])
		}
		fmt.Fprintf(&sb, "\tregisterStructSchema(\"%s\", %s)\n", displayTypeName(name), structSchemaVar)
		fmt.Fprintf(&sb, "}\n")
		...
		if methodsPrefix != "" {
			...
			fmt.Fprintf(&sb, "\tregisterStructSchema(\"%s\", %s)\n", displayTypeName(methodsPrefix), structSchemaVar)
