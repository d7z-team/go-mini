...
`core` 默认提供核心引擎、native `errors` / `fmt.Errorf`，以及 `strings`、`strconv`、`math`、`sort` 这类纯原生值类型标准库 FFI。若需要 io/os/time/fmt/image 等完整标准库 FFI，再通过 `executor.UseSurface(ffilib.Surface())` 装配顶层 surface。
...
公开 FFI schema 额外收紧边界：禁止 `Ptr<T>` 和 `HostRef<Any>`。`Ptr<T>` 只表示 VM 内部 slot 引用，不是 FFI ABI；host identity 必须使用具体 `HostRef<T>` 或明确的 typed interface schema。channel 可以通过 `Chan<T>` / `RecvChan<T>` / `SendChan<T>` 作为 schema endpoint 暴露，但不能装进 `Any`。
...
bytecode JSON 装载执行只依赖 `Executable`。如果 payload 缺少 executable prepared program，运行时装载会拒绝；展示指令和展示元数据不参与执行装载。
...
`AnalysisProgram` 承载 AST、模板 hover 预览和 LSP 查询缓存；`ExecutableProgram` 只承载 bytecode artifact 与 runtime executor，不提供 AST 查询 API。
...
`surface.Bundle` 除了 FFI schema、bind 和 compiler-only 模板，也可以携带纯 VM 源码模块。源码库适合实现 Go-Mini 标准库里不需要 host 权限的部分，例如后续的 VM 版 `context.Context` 包装层；需要 `time.Time`、文件、网络或系统资源时，仍应通过 `ffilib` 提供明确 FFI API。
...
library hash 包含自身源码以及同一个 surface 内导入的 library hash，因此下层纯 VM 库变化会让上层库的 bytecode requirement 失配。不同源码不能注册到同一个 module path；相同源码重复合并是幂等的。

源码库对外只暴露自身源码声明出的函数、变量、常量、类型、结构体和接口。导出表不会继承主程序局部/全局 scope，也不会把默认内建函数或 FFI 注入的常量当成库成员；因此 `mathx.len`、`mathx.math.Pi` 这类不属于库源码的成员会被诊断为不存在。模块对象使用独立的 `TypeModule`，不会再作为 `Any` 参与成员访问或 LSP 类型推断。

### 2.9 Error 语义

VM 的 `Error` 是直接存放在 `TypeError` 中的 Go `error`，不再引入独立 VM error 类型层。`errors.New(...)`、`fmt.Errorf(...)`、`panic(...)` 和 runtime 自身创建的可捕获错误会包装为 `VMStackError`，记录创建点 VM stack；`errors.Stack(err)` 返回这段 VM stack 文本，没有 VM stack 的 host error 返回空字符串。
...
`fmt.Errorf` 复用 Go 的 `%w` 规则。单个 `%w` 可通过 `errors.Unwrap` 取得下一层；多个 `%w` 会形成 Go join-style wrapping，`errors.Is` 会遍历匹配，`errors.Unwrap` 与 Go 标准库一致返回 `nil`。`errors.As` 使用 Go-like 写法：先声明目标变量，再传入 `&target`。VM 侧可表达的目标是 `Error` / `Any` slot；由于 VM 没有 Go 具体错误类型系统，不能把 host 具体错误类型作为 `As` target。

FFI 返回的 Go `error` 会进入 `VMHostError` 包装。包装会保留 host handle/bridge，并在可解析时保留原始 Go error chain，因此 VM 内的 `errors.Is(hostWrapped, hostTarget)` 会走 Go `errors.Is` 语义。`Any` 仍然不能承载带 host handle 的 error；需要跨 FFI 传递 error 时必须在 schema 中显式声明 `Error`。
...
`&x` 只允许用于 VM 可寻址 slot，例如局部变量、全局变量、VM struct 字段和 `*p`。map index、slice expression、host reference、FFI 调用返回值和 opaque host value 都不能取地址。`Ptr<T>` 在 runtime 内部使用独立 pointer 值保存 `*Slot`，不使用 host handle ID，也不能出现在 FFI wire 或 `Any` 中。`Ptr<T>` 与 `T` 之间没有隐式互转，调用和赋值必须显式使用 `&` 或 `*`。
...
`select` 会先尝试已经 ready 的 VM channel 和 FFI channel endpoint；没有 ready case 且存在 `default` 时执行 `default`。没有 ready case 且没有 `default` 时，当前执行上下文会 park。纯 VM channel 等待属于 VM 内部互等；如果所有执行上下文都只剩这类等待，runtime 会返回 `VMAllBlockedError`。FFI channel endpoint 等待属于外部 wake source，可以让调度器继续等待宿主侧 send/receive/close。

host goroutine 可以由 FFI channel endpoint 用来等待宿主 channel 或 I/O，并在完成后唤醒 VM 调度器；它不能执行 VM 指令，也不能持有 VM pointer 作为 host identity。
...
FFI `Any` 只能传递纯值数据，例如 primitive、bytes、array、map 和 VM value struct。它不能承载 host handle、host ref、带 handle 的 error/interface、VM pointer 或 channel；需要传递宿主对象时必须在 schema 中声明具体 `HostRef<T>`，需要传递 error 时必须声明 `Error`，需要传递 channel 时必须声明 `Chan<T>` / `RecvChan<T>` / `SendChan<T>`。MethodID 0 / `Invoke` 只用于已有明确 schema 的 route 或 typed interface method，不提供普通 `HostRef` 的无 schema 动态调用兜底。
...
FFI wire 上不复制 channel 本身，只传 `ffigo.ChannelRegistry` 中的 endpoint ID。生成的 host router 和 Go proxy 会把 Go channel 包装成 `ffigo.ChannelEndpoint`，send/receive payload 继续使用同一套 FFI 编解码。receive-only endpoint 只暴露 `Recv` / `TryRecv`，send-only endpoint 只暴露 `Send` / `TrySend`。
...
生成器会拒绝 bidirectional `chan T` 参数，因为 Go 原生 `chan T` 无法无歧义地代理远端 endpoint 的双向流量；这类 API 应改成 `<-chan T` 或 `chan<- T`。非 proxy surface 可以返回 bidirectional `chan T`，VM 侧会直接得到 `Chan<T>` endpoint；显式 `ffigen:proxy` 不支持返回 bidirectional `chan T`，同样需要使用方向类型。

FFI channel endpoint 的宿主 goroutine 只允许等待宿主 channel、完成 payload 编解码并唤醒 VM 调度器；不能执行 VM task，也不能把 VM pointer 或 channel 放入 `Any`。
