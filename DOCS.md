...
- `PreparedProgram` 持久化显式 `Exports` 表，VM 模块导入按该表构造成员集合
...
公开 FFI schema 使用具体 `HostRef<T>` 或明确的 typed interface schema 表达 host identity。`Ptr<T>` 表示 VM 内部 slot 引用；channel 通过 `Chan<T>` / `RecvChan<T>` / `SendChan<T>` 作为 schema endpoint 暴露；`Any` 面向 primitive、bytes、array、map 和 VM value struct 这类纯值数据。
...
bytecode JSON 装载执行只依赖 `Executable`。如果 payload 缺少 executable prepared program，运行时装载会拒绝；展示指令和展示元数据不参与执行装载。`Executable` 中的显式导出表会和 globals/functions/constants/schema 一起校验，导出目标缺失或类型非法的 bytecode 会在执行前被拒绝。
...
`&x` 用于 VM 可寻址 slot，例如局部变量、全局变量、VM struct 字段和 `*p`。`Ptr<T>` 在 runtime 内部使用独立 pointer 值保存 `*Slot`。`Ptr<T>` 与 `T` 之间通过显式 `&` / `*` 读写。
...
FFI `Any` 面向纯值数据，例如 primitive、bytes、array、map 和 VM value struct。宿主对象、error 和 channel 分别通过具体 `HostRef<T>`、`Error`、`Chan<T>` / `RecvChan<T>` / `SendChan<T>` 在 schema 中表达。MethodID 0 / `Invoke` 用于已有明确 schema 的 route 或 typed interface method。
