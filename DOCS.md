...
- `tuple(A, B)`
- `function(A, B) R`
- `interface{Read(Array<Byte>) tuple(Int64, Error);}`
- `struct { Name String; }`

Go 风格输入如 `[]int`、`*T`、`map[string]int`、`interface{}` 会在 Go 前端转换阶段立即规范化；`byte` / `uint8` 归一为 `Byte`，`rune` 和字符字面量如 `'A'` / `'\n'` / `'\xff'` / `'你'` 归一为 `Rune`。底层运行时仍以 `Int64` 数值存储，只有 `Byte` 在赋值、FFI 和 reflect 写入时校验 `0..255` 范围，`Rune` 不做 Unicode scalar 校验。`[]byte` 归一为 `Array<Byte>`，`[]rune` 归一为 `Array<Rune>`；公开 canonical type 没有独立 bytes 专用类型。其他语言前端应实现 `core/frontend.Frontend` 并输出 canonical type；执行装载只接受 bytecode。
...
FFI `Any` wire 只面向 primitive、`Array<Byte>`、array、map 和 VM value struct 这类纯值数据。VM pointer、`HostRef<T>`、channel、module、closure、host error/interface handle 可以留在 VM `Any` 这类 runtime-only 值路径内，但不会被 FFI `Any` 编码，也不会进入 JSON 纯值或持久化 wire。FFI 返回值按 route schema 从 wire 解码，不反射解构任意 Go host struct、slice、array 或 pointer。

运算和比较遵循统一类型门禁：数值之间可做数值运算和比较，`String` 只和 `String` 做拼接与有序比较，byte slice 使用普通 `Array<Byte>` 的 `append`、index、slice 和 `String([]byte)` 转换语义，不提供独立 bytes 拼接运算。`%` / 位运算 / 位移只接受 `Int64`。不同 primitive 类型不会通过字符串化或 `Any` 通配自动比较；例如 `10 == "10"` 是编译错误，不会求值为 `true` 或 `false`。动态 `Any` 参与比较时会在运行时展开真实值并继续执行同一套规则，类型不匹配会返回 VM 错误。
...
自定义模板的最短使用方式是在执行器上注册 `calltemplate.FunctionTemplate`，然后在脚本里像调用普通函数一样调用模板名。下面示例使用 core 默认提供的 `fmt` 包；如果模板体引用自定义包，需要先注册对应 FFI/schema：
...
`fmt` 是 core 默认注册的 VM 源码库。`Print` / `Println` / `Printf` / `Sprint` / `Sprintf` / `Errorf` 会在 VM 内通过 `reflect` 格式化值，支持 VM struct 值、VM pointer、array、map、`[]byte` 和 `map[string]any` 等复合值。`%c` / `%q` 可格式化 rune/int64。公开 `fmt` 的 VM 值格式化留在 VM 内完成；宿主 helper 只接收已经格式化好的字符串或显式 error causes。

宿主边界只保留 `fmt/internal.Write(text string)` 和 `fmt/internal.Errorf(message string, causes []error)`：前者接收已经格式化完成的字符串并写到输出 sink，后者只负责创建带 VM stack 的 error wrapping。`fmt.Errorf("%w", err)` 的 wrapping 语义保持 Go-like；普通值的字符串化由 VM `fmt` 源码完成。
...
func main() {
    ...
    fmt.Println("user", &u)
    _ = fmt.Sprintf("%v %q", map[string]any{"name": u.Name}, []byte("go"))
}
```
...
VM 源码 `fmt.Errorf` 复用 Go 的 `%w` 规则。单个 `%w` 可通过 `errors.Unwrap` 取得下一层；多个 `%w` 会形成 Go join-style wrapping，`errors.Is` 会遍历匹配，`errors.Unwrap` 与 Go 标准库一致返回 `nil`。`errors.As` 使用 Go-like 写法：先声明目标变量，再传入 `&target`。VM 侧目标使用 `Error` / `Any` slot 表达。
...
- `Zero(typeName) Any`：返回可写入的 VM zero value，便于按字段填充；pointer、channel、HostRef 等类型返回对应 VM nil/zero 值
- `Len(v) Int64`、`Index(v, i) (Any, Bool)`、`MapKeys(v) ([]Any, Bool)`、`MapIndex(v, key) (Any, Bool)`、`MakeMap(typeName) (Any, Bool)`、`SetMapIndex(v, key, value) Error`、`Unwrap(v) (Any, Bool)`、`Elem(v) (Any, Bool)`、`Assign(target, value) Error`、`Append(target, value) Error`
...
`Field`、`Index`、`MapKeys`、`MapIndex`、`Unwrap` 和 `Elem` 返回 VM `Any`，可以承载 pointer、HostRef、channel、module、closure、function 和 interface 等运行时值。array、map、struct 等可变容器以 detached snapshot 返回，修改 snapshot 不会反写原值；pointer、HostRef、channel、module、closure 和 interface 等身份值保持 runtime identity。`Elem` 解 VM pointer 或 typed interface 的当前目标值，nil pointer 和不适用类型返回 `Bool=false`。`MakeMap` 和 `Zero` 创建普通 VM 值，pointer、channel、HostRef 等类型返回对应 VM nil/zero 值。`SetField`、`SetMapIndex`、`Assign` 和 `Append` 都走正常 VM 赋值规则；FFI `Any`、JSON 和持久化边界会单独执行纯值校验。`reflect.Type`、`reflect.StructField`、`reflect.Method`、`reflect.Route`、`reflect.PackageInfo` 和 `reflect.Member` 是只读 metadata struct，不能通过 `SetField` 修改。
...
顶层 `ffilib` 提供的 `encoding/json` 是 VM 源码库实现，不调用 Go 标准库 `encoding/json`，也不使用 Go 原生 reflect。JSON 文本解析、序列化、struct 字段遍历和 tag 处理都在 VM 代码里完成；`Unmarshal` 是普通 VM 源码函数，通过 VM `Any` 承载 pointer target，再使用 VM `reflect.TypeOf` / `reflect.Assign` 完成 typed write-back。

JSON 转换与 tag 处理 helper 都是 `encoding/json` 包内私有实现。
...
`Decode` 返回动态纯值树：object 是 `map[string]any`，array 是 `[]any`，string/bool/null 分别映射为 `string`、`bool`、`nil`，number 统一为 `float64`。JSON string 只接受 JSON 标准转义，`\x` 这类 Go 字符串转义会被拒绝。`Unmarshal` 是普通 package member，可以作为函数值传递；它要求第二个实参运行时必须是 pointer，按 pointer element 类型转换并通过 `reflect.Assign` 写回目标。写入 `int64` 时按原始 JSON integer token 精确解析，decimal/exponent/overflow 都返回错误；未知字段忽略，缺失字段保持零值。
...
VM pointer 是 runtime-only 引用，可以留在 VM slot、指针表达式、VM `Any`、VM array/map 和 channel payload 等 runtime-only 路径内。它不能进入 FFI wire、JSON 纯值、持久化 wire 或 host identity 路径。运行时对地址写入、composite literal、append、map index/delete、slice index 和 channel send 都会按目标声明类型重新校验。
...
- primitive 和 VM struct 按值复制；`Array<Byte>` 与其他 array 一样按引用共享容器，reflect 读取 API 返回 detached snapshot。
...
FFI `Any` 面向纯值数据，例如 primitive、`Array<Byte>`、array、map 和 VM value struct。宿主对象、error 和 channel 分别通过具体 `HostRef<T>`、`Error`、`Chan<T>` / `RecvChan<T>` / `SendChan<T>` 在 schema 中表达。VM pointer、`HostRef<T>`、channel、module、closure、host error/interface handle 都不能通过 FFI `Any` 编码；如果 API 需要这些身份，必须在 schema 中显式建模。MethodID 0 / `Invoke` 用于已有明确 schema 的 route 或 typed interface method。

生成的常量 schema 直接使用 `runtime.ConstInt64`、`runtime.ConstByte`、`runtime.ConstRune`、`runtime.ConstFloat64`、`runtime.ConstString` 或 `runtime.ConstBool`，不会在 runtime 通过反射推断 Go 常量类型。非 primitive 常量应在生成阶段失败。
