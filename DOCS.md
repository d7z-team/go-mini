...
`surface.Bundle` 除了 FFI schema、bind 和 compiler-only 模板，也可以携带纯 VM 源码模块。源码库适合实现 Go-Mini 标准库里可以由 VM 语义表达的部分。标准库 `context` 包就是这个形态：公开 API 由 VM 源码库实现，内部 `context/internal` FFI 只提供 sentinel error 和 timer。
