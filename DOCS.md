...
Go 风格输入如 `[]int`、`*T`、`map[string]int`、`interface{}` 只允许出现在 Go 前端，必须在 `core/gofrontend` 转换阶段立即规范化。其他语言前端应实现 `core/frontend.Frontend`，输出已经规范化的 Mini AST。手写 AST、bytecode 和 FFI schema 不做兼容修复；执行装载只接受 bytecode。

公开 FFI schema 额外收紧边界：禁止 `Ptr<T>` 和 `HostRef<Any>`。`Ptr<T>` 只表示 VM 内部 slot 引用，不是 FFI ABI；host identity 必须使用具体 `HostRef<T>` 或明确的 typed interface schema。
...
FFI `Any` 只能传递纯值数据，例如 primitive、bytes、array、map 和 VM value struct。它不能承载 host handle、host ref、带 handle 的 error/interface 或 VM pointer；需要传递宿主对象时必须在 schema 中声明具体 `HostRef<T>`。MethodID 0 / `Invoke` 只用于已有明确 schema 的 route 或 typed interface method，不提供普通 `HostRef` 的无 schema 动态调用兜底。
...
stdio LSP 使用 full text sync，diagnostics debounce 在 server 侧处理；保存会立即 flush pending diagnostics，关闭文件会取消 pending diagnostics 并清理旧诊断。
